    ...
    <div className="min-h-screen bg-gradient-to-br from-green-950 via-gray-950 to-amber-950 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="text-center mb-8">
          <div className="w-20 h-20 bg-green-600 rounded-3xl flex items-center justify-center text-4xl mx-auto mb-4 shadow-2xl shadow-green-900/50">🌾</div>
          <h1 className="text-3xl font-bold text-white">{t('appName')}</h1>
          <p className="text-green-400 mt-1">{t('appTagline')}</p>
        </div>
        ...
        <div className="bg-gray-900/80 backdrop-blur-xl border border-gray-800 rounded-3xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-green-600 hover:bg-green-500 disabled:bg-green-900 text-white font-bold py-4 rounded-xl transition-all duration-200 active:scale-95 shadow-lg shadow-green-900/50 mt-2"
            >
              {isLoading ? (
        ...
        <div className="mt-4 bg-amber-900/30 border border-amber-800/50 rounded-2xl p-4 text-center">
          <p className="text-amber-400 text-xs font-medium">🎯 {t('demoCredentials')}</p>
        </div>
      </motion.div>
    </div>
  );
