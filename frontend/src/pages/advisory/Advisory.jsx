...
export default function Advisory() {
  ...
  const PRIORITY_CONFIG = {
    ...
    low:    { border: 'border-gray-500/30',  bg: 'bg-gray-500/5',   badge: 'bg-gray-500/20   text-gray-400   border-gray-500/30',   label: t('priorityLow')    },
  };
    ...
    <div className="min-h-screen bg-gray-950 p-4 lg:p-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-white text-4xl font-black">
              {advisories.length > 0 ? Math.max(40, 100 - advisories.filter(a => a.priority === 'urgent').length * 20 - advisories.filter(a => a.priority === 'high').length * 10) : 100}
              <span className="text-xl text-gray-400">/100</span>
            </p>
            <p className="text-amber-400 text-sm mt-1">
              ⚠️ {advisories.filter(a => a.priority === 'urgent' || a.priority === 'high').length} {t('issuesFound')}
            </p>
          </div>
      ...
      {!isLoading && advisories.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-400 text-sm">Click the button above to get AI-powered farm advice</p>
        </div>
      )}
                ...
                <div className="space-y-2 pl-12">
                  {adv.actions.map((action, ai) => (
                    <div key={ai} className="flex items-start gap-2 text-sm text-gray-300">
                      <span className="text-green-500 font-bold mt-0.5">{action.step}.</span>
                      <span>{action.action}</span>
                      {action.timing && (
                        <span className="ml-auto text-xs text-gray-500 capitalize">⏰ {action.timing}</span>
                      )}
                    </div>
                  ))}
                </div>
